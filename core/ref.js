  ...
  function (array, djson, dojo, lang, stamp, dojox) {
    ...
    return dojox.json.ref = {
      ...
      resolveJson: function ( /*Object*/ root, /*Object?*/ args) {
        ...
        var ref, reWalk = [];
        var pathResolveRegex =
          /^(.*\/)?(\w+:\/\/)|[^\/\.]+\/\.\.\/|^.*\/(\/)/;
        var addProp = this._addProp;
        var F = function () {};

        function walk(it, stop, defaultId, needsPrefix, schema,
          defaultObject, parent) {
          // this walks the new graph, resolving references and making other changes
          var i, update, val, id = idAttribute in it ? it[idAttribute] :
            defaultId;
          ...
          while (schema) {
            ...
            if (properties) {
              for (i in it) {
                ...
                if (propertyDefinition && propertyDefinition.format ==
                  'date-time' && typeof it[i] == 'string') {
            ...
            if (it.hasOwnProperty(i)) {
              ...
              if ((typeof val == 'object') && val && !(val instanceof Date) &&
                ...
                if (ref) { // a reference was found
                  ...
                  var path = ref.toString().replace(/(#)([^\.\[])/,
                    '$1.$2').match(
                    /(^([^\[]*\/)?[^#\.\[]*)#?([\.\[].*)?/); // divide along the path
                  if (index[(prefix + ref).replace(pathResolveRegex,
                      '$2$3')]) {
                    ref = index[(prefix + ref).replace(pathResolveRegex,
                      '$2$3')];
                  } else if ((ref = (path[1] == '$' || path[1] == 'this' ||
                      path[1] === '') ? root : index[(prefix + path[1])
                      .replace(pathResolveRegex, '$2$3')])) { // a $ indicates to start with the root, otherwise start with an id
                    // if there is a path, we will iterate through the path references
                    if (path[3]) {
                      path[3].replace(/(\[([^\]]+)\])|(\.?([^\.\[]+))/g,
                        function (t, a, b, c, d) {
                          ref = ref && ref[b ? b.replace(/[\"\'\\]/, '') :
                            d];
                        });
                    }
                  }
                  if (ref) {
                    ...
                    if (!stop) {
                      ...
                      val = walk(val, false, val[refAttribute], true,
                        propertyDefinition);
                  ...
                  if (!stop) { // if we are in stop, that means we are in the second loop, and we only need to check this current one,
                    ...
                    val = walk(
                      ...
                      target != it && typeof target[i] == 'object' &&
                      target[i],
                      it
                    );
                  }
              ...
              if (target != it && !target.__isDirty) { // do updates if we are updating an existing object and it's not dirty
                ...
                if (update && val !== old && // see if it is different
                  ...
                  !(i.charAt(0) == '_' && i.charAt(1) == '_') && i !=
                  "$ref" &&
                  !(val instanceof Date && old instanceof Date && val.getTime() ==
                    old.getTime()) && // make sure it isn't an identical date
                  !(typeof val == 'function' && typeof old == 'function' &&
                    val.toString() == old.toString()) && // make sure it isn't an indentical function
                  index.onUpdate) {
          ...
          if (update && (idAttribute in it || target instanceof Array)) {
            ...
            for (i in target) {
              if (!target.__isDirty && target.hasOwnProperty(i) && !it.hasOwnProperty(
                  i) && !(i.charAt(0) == '_' && i.charAt(1) == '_') && !(
                  target instanceof Array && isNaN(i))) {
                if (index.onUpdate && i != "_loadObject" && i !=
                  "_idAttr") {
                  index.onUpdate(target, i, target[i], undefined); // call the listener for each update
                }
                delete target[i];
                while (target instanceof Array && target.length && target[
                    target.length - 1] === undefined) {
      ...
      fromJson: function ( /*String*/ str, /*Object?*/ args) {
        ...
        var root;
        // try {
          let tmp = eval('(' + str + ')'); // do the eval
					var walkTmp = null;
					walkTmp = function (obj) {
      ...
      toJson: function ( /*Object*/ it, /*Boolean?*/ prettyPrint, /*Object?*/
        idPrefix, /*Object?*/ indexSubObjects, /*String*/ idAttribute) {
        // summary:
          ...
          if (typeof it == 'object' && it) {
            ...
            if (it instanceof Array) {
              var res = array.map(it, function (obj, i) {
                var val = serialize(obj, idAttributeProvided ?
                  undefined : addProp(path, i), nextIndent);
            ...
            var output = [];
            if (idAttributeProvided && (typeof it[idAttribute] ===
                'undefined')) {
              output.push(newLine + nextIndent + djson._escapeString(
                idAttribute) + ":" + sep + djson.toJson(it.__id));
            }
            for (var i in it) {
              if (it.hasOwnProperty(i)) {
                var keyStr;
                ...
                var val = serialize(it[i], idAttributeProvided ?
                  undefined : addProp(path, i), nextIndent);
                if (typeof val != "string") {
                  // skip non-serializable values
                  continue;
                }
                output.push(newLine + nextIndent + keyStr + ":" + sep +
                  val);
              }
            }
      ...
      _addProp: function (id, prop) {
        return id + (id.match(/#/) ? id.length == 1 ? '' : '.' : '#') +
          prop;
      },
      // refAttribute: String
